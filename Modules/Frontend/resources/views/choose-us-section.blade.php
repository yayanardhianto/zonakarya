...
@section('title')
    <title>{{ __('Choose Us Section') }}</title>
@endsection
@php
...
@section('admin-content')
    <div class="main-content">
        <section class="section">
            <x-admin.breadcrumb title="{{ __('Choose Us Section') }}" :list="[
                                ...
                                <ul class="lang_list">
                                    @foreach ($languages as $language)
                                        <li><a id="{{ request('code') == $language->code ? 'selected-language' : '' }}"
                                                href="{{ route('admin.choose-us-section.index', ['code' => $language->code]) }}"><i
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <x-admin.form-title :text="__('Choose Us Section')" />
                                <div>
                                    <x-admin.back-button :href="route('admin.dashboard')" />
                                </div>
                            </div>
                            <div class="card-body">
                                <form action="{{ route('admin.choose-us-section.update', ['code' => $code]) }}"
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <x-admin.form-input id="title" data-translate="true" name="title"
                                                    ...
                                                    value="{{ $chooseUsSection?->getTranslation($code)?->content?->title }}"
                                                    required="true" />
                                            </div>
                                        </div>
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <x-admin.form-editor id="sub_title" name="sub_title"
                                                    label="{{ __('Sub Title') }}" value="{!! $chooseUsSection?->getTranslation($code)?->content?->sub_title !!}"
