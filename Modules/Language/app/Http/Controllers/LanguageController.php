        ...
        if (request('column') == 'is_default') {
            if ( $language->status == 0 ) {
                return response()->json([
                    'status' => false,
                    'message' => __('You can not set an inactive language as the default language'),
                ]);
            }
            ...
            if ( $language->is_default == 1 ) {
                return response()->json([
                    'status' => false,
                    'message' => __('You can not inactive the default language'),
                ]);
            }
            $language->status = $language->status ? 0 : 1;
            if (isset($language) && 0 == $language->status && session()->has('lang') && session()->get('lang') == $language->code) {
                sessionForgetLangChang();
            }
        }
