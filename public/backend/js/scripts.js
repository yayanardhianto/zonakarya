...
// ChartJS
if (window.Chart) {
    Chart.defaults.global.defaultFontFamily = "'Plus Jakarta Sans', 'Nunito', 'Segoe UI', 'Arial'";
    ...
    Chart.defaults.global.tooltips.titleFontFamily =
        "'Plus Jakarta Sans', 'Nunito', 'Segoe UI', 'Arial'";
...
$(function () {
    ...
    var scrollToActiveMenuItem = function() {
        var activeMenuItem = $('.main-sidebar .sidebar-menu li.active');
        console.log('Active menu item found:', activeMenuItem.length);
        
        if (activeMenuItem.length > 0) {
            // Wait for niceScroll to be ready with longer timeout
            var checkNiceScroll = function(attempts) {
                attempts = attempts || 0;
                
                if (sidebar_nicescroll && sidebar_nicescroll.doScrollTop) {
                    console.log('NiceScroll is ready, scrolling to active item');
                    
                    // Get the position of the active menu item relative to the sidebar
                    var activeItemPosition = activeMenuItem.position().top;
                    var sidebarHeight = $('.main-sidebar').height();
                    var itemHeight = activeMenuItem.outerHeight();
                    
                    // Calculate scroll position to center the active item
                    var scrollPosition = Math.max(0, activeItemPosition - (sidebarHeight / 2) + (itemHeight / 2));
                    
                    console.log('Scrolling to position:', scrollPosition);
                    sidebar_nicescroll.doScrollTop(scrollPosition, 800); // 800ms animation
                } else if (attempts < 5) { // Try up to 5 times (1 second total)
                    console.log('NiceScroll not ready, retrying... attempt', attempts + 1);
                    setTimeout(function() {
                        checkNiceScroll(attempts + 1);
                    }, 200);
                } else {
                    console.log('NiceScroll not ready after 5 attempts, using fallback scroll immediately');
                    ...
                    $('.main-sidebar').scrollTop(scrollPosition);
    ...
    // Wait for sidebar to be fully initialized before trying to scroll
    setTimeout(function() {
    ...
    var storeScrollPosition = function() {
        if (sidebar_nicescroll) {
            var currentScroll = sidebar_nicescroll.getScrollTop();
            sessionStorage.setItem('sidebarScrollPosition', currentScroll);
            console.log('Stored scroll position:', currentScroll);
        }
    };
    ...
    var restoreScrollPosition = function() {
        ...
        if (storedPosition) {
            console.log('Restoring scroll position:', storedPosition);
            if (sidebar_nicescroll && sidebar_nicescroll.doScrollTop) {
                sidebar_nicescroll.doScrollTop(parseInt(storedPosition), 0);
            } else {
                // Fallback: use native scroll
                $('.main-sidebar').scrollTop(parseInt(storedPosition));
            }
        }
    };
    
    // Store scroll position when clicking on sidebar links
