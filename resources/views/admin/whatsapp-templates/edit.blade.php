...
@section('title')
    <title>{{ __('Edit WhatsApp Template') }}</title>
@endsection

@section('admin-content')
<div class="main-content">
    <section class="section">
        <x-admin.breadcrumb title="{{ __('Edit WhatsApp Template') }}" :list="[
        ...
        <div class="section-body">
            <div class="card">
                <div class="card-header">
                    <h4>{{ __('Edit WhatsApp Template') }}</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.whatsapp-templates.update', $whatsappTemplate) }}" method="POST">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="name" name="name" 
                                           value="{{ old('name', $whatsappTemplate->name) }}" required>
                                    @error('name')
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="type" name="type" required onchange="updateVariables()">
                                        ...
                                        @foreach($types as $key => $label)
                                            <option value="{{ $key }}" {{ old('type', $whatsappTemplate->type) == $key ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="template" name="template" rows="6" 
                                      placeholder="{{ __('Enter your WhatsApp message template...') }}" required>{{ old('template', $whatsappTemplate->template) }}</textarea>
